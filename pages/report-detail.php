...
<body>
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 md:rounded-2xl shadow-xl p-8 mb-6 text-white">
                <div class="flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div class="flex items-center gap-4"> <?php if (isset($user['role']) && in_array($user['role'], ['SYSTEM', 'ADMIN'])): ?>
                            <button onclick="openDeleteTicketModal()" class="px-4 py-2 bg-red-500/20 hover:bg-red-500 border border-red-400/50 rounded-xl text-white text-sm font-medium transition-all flex items-center gap-2">
            ...
            <div id="deleteTicketModal" class="fixed inset-0 backdrop-blur-md bg-slate-900/70 hidden items-center justify-center z-[11000] p-4">
                <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all">
                    <div class="p-8 text-center">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">ลบรายการแจ้งปัญหา?</h3>
                        <p class="text-gray-500 mb-6 text-sm">การลบจะทำให้ข้อมูลทั้งหมดรวมถึงรูปภาพและประวัติหายไปถาวร กรุณากรอกรหัส <span class="font-bold text-red-600 select-all"><?= htmlspecialchars($reportDetails['code']) ?></span> เพื่อยืนยัน</p>

                        <input type="text" id="confirm_ticket_code" placeholder="พิมพ์รหัส Ticket ที่นี่..."
                            class="w-full px-4 py-3 border-2 border-gray-100 rounded-2xl focus:border-red-500 focus:ring-0 text-center text-lg font-bold tracking-widest mb-6 transition-all">

                        <div class="flex items-center gap-3">
                            <button onclick="closeDeleteTicketModal()" class="flex-1 bg-gray-100 text-gray-700 py-3.5 rounded-2xl font-semibold hover:bg-gray-200 transition-colors">ยกเลิก</button>
                            <button id="btn-confirm-delete-ticket" disabled onclick="deleteTicket()"
                                class="flex-1 bg-red-600 text-white py-3.5 rounded-2xl font-semibold opacity-50 cursor-not-allowed hover:bg-red-700 transition-all shadow-lg shadow-red-200">
                                ยืนยันการลบ
                            </button>
                        </div>
                    </div>
                                ...
                                <div class="mt-6 border-t border-gray-100 pt-5">
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                        <?php foreach ($reportDetails['images'] as $img):
                                            $imgPath = htmlspecialchars($img['file_path']);
                                        ?>
    ...
    <?php if ($canEditStatus): ?>
        ...
        <script>
            ...
            const targetTicketCode = "<?= htmlspecialchars($reportDetails['code']) ?>";

            function openDeleteTicketModal() {
                const modal = document.getElementById('deleteTicketModal');
                modal.classList.remove('hidden');
                modal.classList.add('flex');
                document.getElementById('confirm_ticket_code').focus();
            }
            ...
            function closeDeleteTicketModal() {
                ...
                modal.classList.add('hidden');
                modal.classList.remove('flex');
                document.getElementById('confirm_ticket_code').value = '';
            }

            document.getElementById('confirm_ticket_code').addEventListener('input', function(e) {
                const btn = document.getElementById('btn-confirm-delete-ticket');
                if (e.target.value === targetTicketCode) {
                    btn.disabled = false;
            ...
            function deleteTicket() {
                ...
                fetch('/api/reports/delete_ticket.php', {
                        ...
                        body: JSON.stringify({
                            id: ticketId,
                            code: targetTicketCode
                        })
                    })
                    .then(r => r.json())
                    .then(d => {
                        if (d.ok) {
                            // ลบสำเร็จ ให้เด้งกลับไปหน้าหลักพร้อม Toast
                            sessionStorage.setItem('toast_msg', 'ลบรายการเรียบร้อยแล้ว');
                            sessionStorage.setItem('toast_type', 'success');
                            window.location.href = './?page=reports';
                        } else {
                            showToast(d.message || 'เกิดข้อผิดพลาดในการลบ', 'error');
                        }
                    })
                    .catch(e => showToast('Error: ' + e.message, 'error'));
            }
